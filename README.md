...
In the notification's body, place a link to a page that handles the opt-in. To the link, append
a query parameter `?rb-token=##token##` where `##token##` is the token generated by the bundle.
